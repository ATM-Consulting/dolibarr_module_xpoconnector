...
class XPOConnector extends SeedObject
{
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'XPOConnectorStatusShortCanceled'
		,self::STATUS_DRAFT => 'XPOConnectorStatusShortDraft'
		,self::STATUS_VALIDATED => 'XPOConnectorStatusShortValidated'
//		,self::STATUS_REFUSED => 'XPOConnectorStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'XPOConnectorStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'xpoconnector';
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->XPOCONNECTOR_REF_MASK) ? $conf->global->XPOCONNECTOR_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'xpoconnector', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("ShowXPOConnector") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/xpoconnector/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='xpoconnector@xpoconnector';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('XPOConnectorStatusCancel'); $statusLabelShort=$langs->trans('XPOConnectorStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('XPOConnectorStatusDraft'); $statusLabelShort=$langs->trans('XPOConnectorStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('XPOConnectorStatusValidated'); $statusLabelShort=$langs->trans('XPOConnectorStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('XPOConnectorStatusRefused'); $statusLabelShort=$langs->trans('XPOConnectorStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('XPOConnectorStatusAccepted'); $statusLabelShort=$langs->trans('XPOConnectorStatusShortAccepted'); }
